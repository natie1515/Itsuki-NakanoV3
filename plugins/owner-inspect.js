...
// ⓘ Variables de emoji - Para el sistema de inspección imperial
const emoji = '🔍'
const emoji2 = '⚠️'
const emoji3 = '⏳'
const msm = '❌'
const md = 'https://whatsapp.com'

let handler = async (m, { conn, command, usedPrefix, args, text, groupMetadata, isOwner, isROwner }) => {
    // ⓘ Verificación de autoridad - Solo para el soberano
    if (!isOwner) return conn.reply(m.chat, 
        `ⓘ \`COMANDO RESTRINGIDO\` 👑\n\n` +
        `ⓘ \`Este sistema de inspección es exclusivo para el soberano.\`\n` +
        `ⓘ \`Solo el dueño del imperio puede acceder a estas funciones.\``,
        m
    )
    ...
    // ⓘ Sistema de contacto para reportes
    async function makeFkontak() {
        try {
            ...
            return {
                ...
                message: { locationMessage: { name: 'Sistema de Inspección Imperial', jpegThumbnail: thumb2 } },
    ...
    async function reportError(e) {
        try {
            await conn.reply(m.chat, `${msm} Ocurrió un error en la operación, mi señor.`, (await makeFkontak()) || m)
        } catch {}
        console.log(e)
    }
    ...
    switch (true) {     
        case isCommand1:
            ...
            if (!text) return await conn.reply(m.chat, 
                `${emoji} \`Ingrese el enlace de grupo/comunidad o canal, mi señor.\``, 
                (await makeFkontak()) || m
            )
            ...
            const MetadataGroupInfo = async (res, isInviteInfo = false) => {
                ...
                let caption = 
                    ...
                    `ⓘ \`Inspección completada. Todos los datos han sido analizados, mi señor.\` ✅`
            ...
            const inviteGroupInfo = async (groupData) => {
                ...
                let caption = 
                    ...
                    `ⓘ \`Inspección remota completada, mi señor. Información obtenida exitosamente.\``
            ...
            try {
                ...
                console.log('ⓘ Método de metadatos - Inspección interna')
            } catch {
                ...
                if (inviteUrl) {
                    try {
                        ...
                        console.log(`${emoji3} Método de enlace - Inspección remota.`)    
                    } catch (e) {
                        await conn.reply(m.chat, 
                            `${emoji2} \`Grupo no encontrado, mi señor.\``, 
                            (await makeFkontak()) || m
                        )
                        return
                    }
            ...
            if (info) {
                await conn.sendMessage(m.chat, { 
                    ...
                    contextInfo: {
                        ...
                        externalAdReply: {
                            title: `${emoji} ɪɴsᴘᴇᴄᴛᴏʀ ɪᴍᴘᴇʀɪᴀʟ`,
                            body: `${emoji2} ᴇxᴄʟᴜsɪᴠᴏ ᴘᴀʀᴀ ᴇʟ sᴏʙᴇʀᴀɴᴏ`,
                ...
                if (!channelUrl) return await conn.reply(m.chat, 
                    `${emoji2} \`Verifique que sea un enlace de canal de WhatsApp, mi señor.\``, 
                    (await makeFkontak()) || m
                )
                ...
                if (channelUrl) {
                    try {
                        ...
                        if (!newsletterInfo) return await conn.reply(m.chat, 
                            `${emoji2} \`No se encontró información del canal, mi señor. Verifique que el enlace sea correcto.\``, 
                            (await makeFkontak()) || m
                        )
                        ...
                        const caption = `ⓘ \`INSPECCIÓN DE CANAL IMPERIAL\` 📢\n\n` + processObject(newsletterInfo, '', newsletterInfo?.preview)
                            ...
                            viewOnceMessage: {
                                message: {
                                    ...
                                    interactiveMessage: proto.Message.InteractiveMessage.fromObject({
                                        ...
                                        footer: proto.Message.InteractiveMessage.Footer.create({ text: 'ⓘ Toca un botón para copiar la información estratégica.' }),
                                        header: proto.Message.InteractiveMessage.Header.fromObject({ 
                                            title: 'ENLACES IMPERIALES GENERADOS', 
